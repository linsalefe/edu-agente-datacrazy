...
class CRMSyncService:
    ...
    # -------------------------
    # 🆕 ENVIAR MENSAGEM VIA DATACRAZY (para aparecer nas conversas)
    # -------------------------
    def send_message_via_crm(self, conversation_id: str, message: str) -> bool:
        """
        Envia mensagem através do DataCrazy (para aparecer nas conversas do CRM)
        ...
        try:
            if not conversation_id:
                logger.warning("⚠️  conversation_id não fornecido para send_message_via_crm")
                return False
            
            if not message or not message.strip():
                logger.warning("⚠️  Mensagem vazia, ignorando send_message_via_crm")
                return False
            
            result = self.crm.send_message(conversation_id, message)
            
            if result:
                logger.info(f"✅ Mensagem enviada via DataCrazy (conversa {conversation_id})")
                return True
            
            logger.error(f"❌ Falha ao enviar mensagem via DataCrazy (conversa {conversation_id})")
            return False
        
        except Exception as e:
            logger.exception(f"❌ Erro ao enviar mensagem via CRM: {e}")
            return False